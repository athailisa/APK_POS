<nav class="navbar navbar-expand-lg">
    <div class="container-fluid">
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            ...
            <div class="dropdown">
                <button class="btn dropdown-toggle d-flex align-items-center gap-2 user-menu-btn" type="button"
                    data-bs-toggle="dropdown" aria-expanded="false">
                    <span class="user-avatar">{{ strtoupper(substr(auth()->user()->name, 0, 1)) }}</span>
                    <span class="text-start d-none d-md-block">
                        <span class="d-block fw-semibold"
                            style="font-size:0.9rem; line-height:1.1;">{{ auth()->user()->name }}</span>
                        <span class="d-block text-muted"
                            style="font-size:0.75rem;">{{ ucfirst(auth()->user()->role->name) }}</span>
                    </span>
                </button>
